...
export function Card({ challenge }: { challenge: ChallengeObjectFull }) {
  return (
    <motion.div
      layout="position"
      className="relative rounded-lg border-1 border-gray-200 p-2"
    >
      <Link to={`/challenge/${slugify(challenge.name)}-${challenge._id}`}>
        <Image
          className="h-auto w-full overflow-hidden rounded-md object-cover object-top"
          src={`${ENV.S3_BUCKET_PUBLIC_URL}/${challenge.images[0].key}`}
        />
      </Link>
      <div className="px-2">
        <h2 className="mt-3 text-sm font-semibold">{challenge.name}</h2>
        <div className="mt-3 flex items-center justify-start gap-2 text-xs">
          <Image
